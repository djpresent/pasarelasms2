				...
				<form onsubmit="return verificar();" action="/reporteTransacciones">
        			...
        			<div class="row">
        				...
        				<!--
        				 ...
        				 <div class="col-xs-3">
        					<div class="form-group">
        						<label for="rServicio">Servicios</label>
        							<%
									...
									if (!(session.getAttribute("ser")==null))
									{
										...
										for( Servicio servicio:lista) 
										{
											...
											if(!(session.getAttribute("ser").equals("noservicio")) )
											{
												//System.out.println(usuario.getNombres()+" "+ session.getAttribute("usu"));
												if (servicio.getIdServicio()==Integer.parseInt(session.getAttribute("ser").toString()))
												{
													%>
														<option value=<%= servicio.getIdServicio()  %> selected><%= servicio.getDescripcion() %></option>
													<% 
												}
												else
												{
													%>
													<option value=<%= servicio.getIdServicio() %>><%= servicio.getDescripcion() %></option>
													<% 
												}
											}
											else
